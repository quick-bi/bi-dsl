...
> 时间趋势 / 连续变化。范式：encoding。

本文件包含（5）：`LineChartOptions` / `LineChartSpec` / `LineOptions` / `PolylineChartOptions` / `PolylineChartSpec`

---
## LineChartOptions

<!-- DOCS:AUTO START field-table:LineChartOptions -->
...
<!-- DOCS:AUTO START field-table:LineChartSpec -->
...
| `type` | `"line"` | ✅ | 图表类型：普通折线图 |
| `options` | `{ data_zoom, smooth, connect_nulls, show_symbol, symbol_type, line_width, line_type, area_opacity, area_gradient, dual_axis }` |  | 折线图选项配置 |

<!-- DOCS:AUTO END -->

## LineOptions

<!-- DOCS:AUTO START field-table:LineOptions -->
...
## PolylineChartOptions

<!-- DOCS:AUTO START field-table:PolylineChartOptions -->
...
| `area_gradient` | `boolean` |  | 面积是否开启渐变填充 |
| `dual_axis` | `{ enable, sync_mode }` |  | 双 Y 轴配置 |
| `stack_mode` | `"group" \| "stack" \| "percent"` |  | 堆叠模式（面积图子类型）：group=分组, stack=堆叠, percent=百分比堆叠 |

<!-- DOCS:AUTO END -->

## PolylineChartSpec

<!-- DOCS:AUTO START field-table:PolylineChartSpec -->
...
| `color` | `{ theme, colors }` |  | 配色 |
| `analysis` | `{ reference_line }` |  | 分析增强配置， 辅助线reference_line |
| `type` | `"polyline"` | ✅ | 图表类型：面积图 |
| `options` | `{ data_zoom, smooth, connect_nulls, show_symbol, symbol_type, line_width, line_type, area_opacity, area_gradient, dual_axis, stack_mode }` |  | 面积图选项配置 |
